		...
		if GPIO.input(16) == 0:
			...
			for id in process:
				...
				if proc.name() == "fbi":
					...
					myprocessfbi = subprocess.Popen(['fbi','-a','--noverbose','-T','1','/home/pi/onair.16.9.jpg'], stdin=subprocess.PIPE)
					time.sleep(.1)
			...
			#test to see if OMXPlayer (video player) is running
				...
				if proc.name() == "omxplayer":
					...
					myprocessomx = subprocess.Popen(['omxplayer','-b','--loop','/home/pi/video.mp4'],stdin=subprocess.PIPE)
					time.sleep(.1)
